...
namespace Test_DLL_TomsRotaryCipher
{
    class Program
    {
        ...
        public static void HidingInPlainSight(byte[] bIn)
        {
            ...
            TomsRotaryCipher.PopulateSeeds(); 
            TomsRotaryCipher.oSettings.MovingCipherRotors = 3;
        ...
        public static void TestUsingSigabaSecureXOR(byte[] bIn)
        {
            ...
            TomsRotaryCipher oTRC = new TomsRotaryCipher();
            TomsRotaryCipher.PopulateSeeds(); // generates required 32 bit words for seeding lookup tables using RNGCryptoServiceProvider
            // rotors are generated with MS' version of a PRNG: System.Random.
            TomsRotaryCipher.oSettings.MovingCipherRotors = 5;// Sigaba with CBCMode and SecureXOR permits a smaller rotor definition.
            byte[] XORdSecure = TomsRotaryCipher.SecureXOR(bIn); // additional layer using XOR, oSeeds.SeedXOR is used to create OTP, generated by PRNG.
            ...
            byte[] bAllSettings = TomsRotaryCipher.GetAll();
            ...
            TomsRotaryCipher.LoadAll(File.ReadAllBytes("SettingsForAlice.bin"));
            ...
            byte[] bDecodedPlainTxt = oTRC_Alice.SAES(TomsRotaryCipher.oSettings.NotchPlan, bCipherTxt,
                TomsRotaryCipher.oSettings.EnigmaMode,
                oTRC_Alice.GetCorrectDecodeOpt(TomsRotaryCipher.oSettings.NoReflectorMode),
                oTRC_Alice.GetCorrectDecodeOpt(TomsRotaryCipher.oSettings.CBCMode));
            // final XOR to recover plaintext:
            byte[] bCipherTxtNew = TomsRotaryCipher.SecureXOR(bDecodedPlainTxt);
        ...
        public static void TestUsingMaxKeyspace(byte[] bIn)
        {
             ...
             */
            TomsRotaryCipher oTRC = new TomsRotaryCipher();
            TomsRotaryCipher.PopulateSeeds(); // generates required 32 bit words for seeds using RNGCryptoServiceProvider
            // rotors are generated with MS' version of a PRNG: System.Random.
            TomsRotaryCipher.oSettings.MovingCipherRotors = 1000; 
            byte[] bCipherTxt = oTRC.SAES(NotchPlan.Sequential,
                bIn, // plaintext
                EnigmaMode.NoReflector, // best security, using Reflector omits character ID. (Note, the Reflector option is included for educational and historical reasons)
                NoReflectorMode.Reverse, // choose a direction, does not matter which way.
